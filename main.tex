...
\date{\vspace{-0.8em}\today\vspace{-0.6em}}
...
\item derive an analytic expression for the ensemble variance (uncertainty) across graphs, and
...
The following identities are direct algebra from the model (included for completeness and to anchor the unknown-graph setting).
...
\paragraph{Interpretation (strong claim).}
...
Thus the variance-based estimator is \emph{not a heuristic}: it is exactly the ensemble mean under the uniform-stub-matching (maximum-entropy-over-matchings) graph model.
...
For the toy experiments we simply fix $\ell$ exactly; for larger demos we recommend either (i) a simple two-level construction or (ii) a parametric prior (e.g., baseline-1 Dirichlet--multinomial \cite{mosimann1962,minka2000}) that can be tuned to match $\sigma^2$.

\textbf{TODO (dev):} implement a robust ``moment-matching'' sampler for $\ell$ given $(T,n,\sigma^2)$ with integer constraints and $\ell_i\ge 1$.
...
\quad\text{(degrees as equal as possible)},
\]
\[
\sigma^2_{\max} = \frac{(n-1)(T-n)^2}{n^2}
\quad\text{(one dominant emitter)}.
\]
\textbf{TODO (paper polish):} add a short proof or cite a discrete variance bound.

\paragraph{How to set $\sigma^2$ in practice.}
% TODO: check this rewritten section with ChatGPT Pro—new framing.
...
An important next step is to study robustness to structured alternatives.
...
\paragraph{Aggregating many weak matching signals.}
In realistic datasets, we may observe many collisions across many value labels, each providing a weak same-source signal.
A promising direction is to integrate value-conditioned posteriors across multiple observed matches to form a stronger overall estimate of same-source probabilities (e.g., by treating per-collision posteriors as evidence factors in a probabilistic record-linkage or clustering objective, or by fitting a simple latent-variable model whose sufficient statistics include the observed collision counts).
Working out a principled aggregation scheme (and its calibration/uncertainty) is left to future work.

\paragraph{Multiple independent emission fields.}
...
\paragraph{Extending to $k$-wise collisions.}
The present analysis considers pairs of draws.
When $k>2$ draws all produce the same value label, the event carries stronger evidence of a shared latent source.
Extending Theorem~\ref{thm:mean} to a $k$-wise collision posterior $P(E_1=\cdots=E_k\mid V_1=\cdots=V_k)$ would require higher-order stub-matching moments ($p_k$, falling-factorial entity moments $M_k$) but may follow the same proof strategy.
Such results could be useful in settings with high observation multiplicity, such as sensor networks or web-traffic analysis.

\paragraph{Sampling without replacement and finite-population corrections.}
The measurement model assumes draws with replacement.
In some applications the draw process is closer to sampling without replacement (e.g., each edge can be ``used'' at most once).
The without-replacement analogue replaces $P(S)=\alpha$ with a hypergeometric-flavoured calculation, and the configuration-model expectations acquire finite-population correction factors.
Deriving the without-replacement counterparts of Theorems~\ref{thm:mean}--\ref{thm:var} is a natural extension.

\paragraph{Bayesian treatment of $\sigma^2$ and model averaging.}
Rather than conditioning on a point value of $\sigma^2$, one could place a prior over the feasible $\sigma^2$ interval (or equivalently over $\alpha$) and integrate the point estimator against this prior.
Because $\widehat\theta$ is monotone and smooth in $\sigma^2$ (Figure~\ref{fig:sigma-sweep}), the resulting posterior predictive distribution of $\theta$ would inherit tractability.
This Bayesian extension would naturally produce credible intervals that incorporate uncertainty about entity heterogeneity itself, complementing the graph-ensemble uncertainty captured by Theorem~\ref{thm:var}.
