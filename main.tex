...
We give a closed-form estimator for this \emph{collision-conditioned same-source probability} that requires only the observed value frequencies and a single scalar summary of source heterogeneity---the across-source degree variance---with no knowledge of the full source--value emission structure.

The setting is a bipartite multigraph whose hidden bottom vertices are latent entities and whose top vertices are observable value labels; a measurement samples an edge uniformly and returns the value endpoint.
Under a maximum-entropy (configuration-model) ensemble over graphs consistent with the observed value marginals, we prove that the ensemble mean of the exact same-source posterior depends on the unknown entity side only through the entity-degree second moment, equivalently the degree variance.
We also derive an analytic expression for the ensemble variance (uncertainty) of the posterior, which depends on one additional degree-shape statistic.
We validate both formulas by exact enumeration in small cases and by end-to-end simulations that sample ground-truth graphs, estimate value marginals from finite data, and compare the true posterior to the estimator's mean and uncertainty bands.
\end{abstract}
