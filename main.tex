...
\textbf{Jason Bono}\\
Lucor (\url{lucor.ai})\\
\href{mailto:jasonbono@lucor.ai}{\texttt{jasonbono@lucor.ai}}\\[2pt]
{\small ORCID: \href{https://orcid.org/0000-0002-3018-714X}{0000-0002-3018-714X}}
\vspace{-0.3em}
}
\date{\vspace{-0.3em}February 2026\vspace{-0.6em}}

\begin{document}
\maketitle

\begin{abstract}
When two observations share a value, how likely is it that they came from the same latent source?
This question underlies entity resolution, record linkage, and any setting where latent entities emit observable values with heterogeneous intensities.
Practitioners have long known that rare values are more informative than common ones and have built empirical adjustments accordingly, yet a closed-form account of \emph{which summary of entity heterogeneity controls the answer} has been missing.

We model the emission process as a bipartite multigraph connecting entities to value labels and derive a closed-form estimator for the collision-conditioned same-source probability $P(\text{same source}\mid\text{value collision})$.
The surprise is one of simplicity: under a maximum-entropy (configuration-model) ensemble consistent with observed value marginals, the ensemble mean of the exact posterior depends on the unknown entity side only through a single scalar---the across-entity degree variance~$\sigma^2$.
No knowledge of the full entity--value emission structure is needed; $\sigma^2$ acts as a sufficient statistic for the entity-degree sequence.
We also derive an analytic expression for the ensemble variance (uncertainty) of the posterior, which depends on one additional degree-shape statistic, and validate both formulas by exact enumeration in small cases and by end-to-end simulations.
\end{abstract}
...
Examples include repeated measurements of latent sources, anonymized logs, emissions from hidden actors, or sensors that return coarse labels.
...
The question is fundamental to entity resolution and record linkage, where practitioners have long known that rare values carry more evidential weight than common ones (modern linkage systems incorporate empirical term-frequency adjustments \cite{winkler1989b}), but a closed-form answer to \emph{which summary of entity heterogeneity controls the same-source probability} has been missing.
...
We assume the observer can estimate the value-label marginal distribution (equivalently, value-side degrees up to scale) and that domain knowledge provides a summary of entity-side heterogeneity; here, the across-entity degree variance.
We then:
...
These models work with bipartite graphs linking observations to latent entities (structurally the same object as our emission graph) but use them as scaffolding for MCMC inference on the linkage structure rather than deriving closed-form moments of the collision process.
Our goal is complementary: a single scalar summary---the collision-conditioned same-source probability---derived analytically under a principled random-graph null (configuration model) consistent with observed value marginals.
...
\paragraph{Sanity check: \texorpdfstring{$\alpha$}{alpha} is the unconditional same-source probability.}
...
\subsection{Point estimator from \texorpdfstring{$(o,n,\sigma^2)$}{(o, n, sigma-squared)}}
...
The vertical gap between the two curves is the ``collision lift,'' i.e., the additional information gained by conditioning on a value collision at each heterogeneity level.
...
\subsection{Toy set A: \texorpdfstring{$n=3$, $m=3$, $o=(4,4,4)$}{n=3, m=3, o=(4,4,4)}}
...
\subsection{Toy set B: skewed value marginals \texorpdfstring{$o=(8,2,2)$}{o=(8,2,2)} with \texorpdfstring{$\ell=(4,4,4)$}{l=(4,4,4)}}
Here $T=12$ and $D=72$.
...
The dashed line marks the ensemble mean; the dotted line marks the $\sigma^2$-only point estimate (Theorem~\ref{thm:mean}), and the two coincide exactly.
...
\paragraph{How to set \texorpdfstring{$\sigma^2$}{sigma-squared} in practice.}
...
If domain knowledge places $\sigma^2$ in a narrow subinterval, the corresponding vertical slice gives tight bounds and the conclusion is robust.
...
As an additional check, we validate the measurement model end-to-end by simulating draw \emph{pairs} from a known graph $C^*$ and computing the empirical same-source rate among collisions.
For each pair, we draw two edges independently and uniformly from $C^*$, observe both the value endpoints (to detect a collision) and the entity endpoints (to check same-source status, which would be latent in practice but is available in simulation).
...
Whether the ensemble mean of the joint posterior admits a tractable ``variance-only'' parameterisation is an open question.
...
\paragraph{Inverting collisions to estimate \texorpdfstring{$\sigma^2$}{sigma-squared}.}
...
Formalising this inverse problem (identifiability conditions, finite-sample rates) would make the framework fully data-driven.
...
The entity-resolution and random-graph communities each possess the ingredients for the result in this paper: the bipartite entity-to-value structure is standard in Bayesian ER, and the configuration-model moment calculations are standard in random graph theory.
That $P(S\mid\coll)$ reduces to the degree variance $\sigma^2$ is therefore not a deep technical achievement but a short bridge between two fields that have not previously been connected in this way.
We view the simplicity as the point: a single, interpretable scalar replaces either the full (unobserved) emission graph or the empirical frequency adjustments that practitioners currently calibrate from labeled data.
...
\bibitem{winkler1989b}
W.~E. Winkler.
\newblock Frequency-based matching in the {F}ellegi--{S}unter model of record linkage.
\newblock In \emph{Proceedings of the Section on Survey Research Methods},
  pages 778--783. American Statistical Association, 1989.
